...
impl<'data> CoreFileParser<'data> {
        ...
        match &self.object {
            object::File::MachO64(macho_file) => {
                ...
                for load_command_result in macho_file.macho_load_commands()? {
                    ...
                    if let Ok(LoadCommandVariant::Thread(_thread_command, thread_data)) =
                        load_command_data.variant()
                    {
    ...
    fn parse_thread_states_from_data(
        &self,
        data: &[u8],
    ) -> Result<Vec<ThreadState>, Box<dyn std::error::Error>> {
    ...
    pub fn format_thread_state_verbose(&self, thread_state: &ThreadState) -> String {
        ...
        output.push_str(&format!(
            "    State: flavor=0x{:x}{}, count={}, data_len={}\n",
            thread_state.flavor,
            state_type,
            thread_state.count,
            thread_state.state_data.len()
        ));
