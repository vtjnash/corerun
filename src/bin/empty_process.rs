...
use mach2::message::{
    MACH_MSG_TYPE_COPY_SEND, MACH_MSGH_BITS, MACH_MSGH_BITS_COMPLEX, mach_msg_body_t,
    mach_msg_header_t, mach_msg_port_descriptor_t, mach_msg_send,
};
...
fn handle_ipc_commands(ipc_fd: RawFd, coredump_fd: RawFd) {
    ...
    loop {
        ...
        let command: IpcCommand =
            unsafe { std::ptr::read(command_bytes.as_ptr() as *const IpcCommand) };
                ...
                if command_data.len() >= 36 {
                    ...
                    unsafe {
                        ...
                        if mapped_addr == libc::MAP_FAILED {
                            ...
                            eprintln!(
                                "Failed to map segment at 0x{:x}, size: 0x{:x}, offset: 0x{:x}: {} (errno: {})",
                                address,
                                size,
                                file_offset,
                                error,
                            ...
                            eprintln!(
                                "Segment at 0x{:x} mapped to 0x{:x} instead, size: 0x{:x}, offset: 0x{:x}",
                                address, mapped_addr as usize, size, file_offset
                            );
                        } else {
                            eprintln!(
                                "Segment at 0x{:x} mapped successfully, size: 0x{:x}, offset: 0x{:x}",
