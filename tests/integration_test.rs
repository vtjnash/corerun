...
fn apply_entitlements_to_empty_process() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let verify_result = Command::new("codesign")
        .args(["--display", "--entitlements", "-", empty_process_path])
        .output();
...
fn test_core_dump_generation_and_loading() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut child = Command::new(empty_process_path).spawn()?;
    ...
    println!(
        "Successfully parsed {} segments from core dump",
        segments.len()
    );
    ...
    assert!(
        !segments.is_empty(),
        "Core dump should contain at least one segment"
    );
...
fn test_empty_process_with_fd3() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut process_controller = corerun::spawn_empty_process!(coredump_file.as_raw_fd())?;

    println!(
        "Successfully spawned empty process with PID: {}",
        process_controller.get_pid()
    );
    ...
    let thread_commands = Vec::new();
    match process_controller.suspend_threads_with_state(&thread_commands, false) {
        Ok(_) => println!("✓ Successfully suspended threads"),
...
fn test_process_controller_fallback() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match corerun::process_control::ProcessController::spawn_target_process(
        coredump_file.as_raw_fd(),
    ) {
        Ok(controller) => {
...
fn test_segment_parsing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for segment in &segments {
        println!(
            "Segment: {} - addr: 0x{:x}, size: 0x{:x}, offset: 0x{:x}",
